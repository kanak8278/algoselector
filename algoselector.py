...
from rl_questions import RLQuestions, RLAnswers, RLQuestionName
...
class AlgoSelectorWizard(object):
    ...
    def __init__(self):
        ...
        self.rl_questions = RLQuestions()
        self.rl_answers = RLAnswers()
        self.rl_name = RLQuestionName()
    ...
    def reinforced_wizard(self, name, question, description=""):
        """
        The Reinforced Learning Wizard
        """

        return wiz.PromptWizard(
            name=bcolors.OKBLUE + name + bcolors.ENDC,
            ...
            steps=(
                ...
                wiz.WizardStep(
                    # ID where the value will be stored
                    id="answer",
                    name=bcolors.HEADER + question + bcolors.ENDC,
                    help="Y/N/U - Yes/No/Unknown",
                    validators=(wiz.required_validator),
                    default='Y'
                ),
    ...
    def run_mainwiz(self):
        ...
        self.main_l1_values = self.main_wizard_l1().run(self.shell)
        if self.main_l1_values['data_availability'].lower() == 'y':

            self.main_l2b_values = self.main_wizard_l2_b().run(self.shell)
            if self.main_l2b_values['data_programmability'].lower() == 'y':
                ...
                self.main_l3_values = self.main_wizard_l3().run(self.shell)
            ...
            self.main_l2a_values = self.main_wizard_l2_a().run(self.shell)
            if self.main_l2a_values['data_creativity'].lower() == 'y':
                print(bcolors.OKGREEN + "Looks like you need ML, let's continue" + bcolors.ENDC)
                self.rl_01_values = self.reinforced_wizard(self.rl_name.q01, self.rl_questions.q01).run(self.shell)
                if  self.rl_01_values['answer']== 'y':
                    print(bcolors.OKGREEN + self.rl_answers.q01['true'] + bcolors.ENDC)
                else:
                    print(bcolors.FAIL + self.rl_answers.q01['false'] + bcolors.ENDC)
            else:
                print(bcolors.FAIL + "ML is not required - Please consider alternate approaches\n" + bcolors.ENDC)
